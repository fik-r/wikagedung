...
import KegiatanPers from "@/components/kegiatan-pers/KegiatanPers";
...
export default async function Index() {
    ...
    const [dataMenuHeader, dataHomepage, dataContact, dataContent] =
        await Promise.all([
            getMenuHeader(), getHomepageData(), getContact(), getMenuContentByAlias("kegiatan-pers")])

    const content = () => {
        return (
            <KegiatanPers data={dataContent.data}/>
        )
    }
    return (
        <Layout
